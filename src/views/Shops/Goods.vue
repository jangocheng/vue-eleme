<template>
    <div class="goods" v-if="shopInfo">
        <!-- 商家推荐 -->
        <div class="recommend" v-for="(recommend, index) in shopInfo.recommend" :key="index">
            <p class="recommend-name">{{ recommend.name }}</p>
            <div class="recommend-wrap">
                <ul>
                    <li v-for="(item, index) in recommend.items" :key="index">
                        <img :src='item.image_path != "" ? item.image_path : require("../../assets/error.png")' alt />
                        <div class="recommend-food">
                            <p class="recommend-food-name">{{ item.name }}</p>
                            <p class="recommend-food-zm">
                                月售{{ item.month_sales }} 好评率{{ item.satisfy_rate }}
                            </p>
                        </div>
                        <div class="recommend-food-price">
                            <p>¥{{ item.lowest_price }}</p>
                            <CartControll :food="item" />
                        </div>
                    </li>
                </ul>
            </div>
        </div>

        <!-- 商品分类 -->
        <div class="menuview" id="menuview">
            <!-- 左侧分类列表 -->
            <div class="menu-wrapper" ref="menuScroll">
                <ul>
                    <li
                        :class="{ current: currentIndex === index }"
                        @click="selectMenu(index)"
                        v-for="(item, index) in shopInfo.menu"
                        :key="index"
                    >
                        <img v-if="item.icon_url" :src="item.icon_url" alt />
                        <span>{{ item.name }}</span>
                    </li>
                </ul>
            </div>

            <!-- 右侧商品内容 -->
            <div class="foods-wrapper" ref="foodScroll">
                <ul>
                    <li class="food-list-hook" v-for="(item, index) in shopInfo.menu" :key="index">
                        <!-- 内容上 -->
                        <div class="category-title">
                            <strong>{{ item.name }}</strong>
                            <span>{{ item.description }}</span>
                        </div>
                        <!-- 内容下 -->
                        <div
                            @click="handleFood(food)"
                            class="fooddetails"
                            v-for="(food, i) in item.foods"
                            :key="i"
                        >
                            <!-- 左 -->
                            <img :src='food.image_path != "" ? food.image_path : require("../../assets/error.png")' alt />
                            <!-- 右 -->
                            <section class="fooddetails-info">
                                <h4>{{ food.name }}</h4>
                                <p class="fooddetails-des">{{ food.description }}</p>
                                <p class="fooddetails-sales">
                                    月售{{ food.month_sales }}份 好评率{{ food.satisfy_rate }}
                                </p>
                                <div class="fooddetails-price">
                                    <span class="price">¥{{ food.lowest_price }}</span>
                                    <CartControll :food="food" />
                                </div>
                            </section>
                        </div>
                    </li>
                </ul>
            </div>
        </div>

        <!-- 购物车 -->
        <ShopCart :shopInfo="shopInfo" />

        <!-- 商品详情 -->
        <FoodDetails :food="selectedFood" :isShow="showFood" @close="showFood = false" />
    </div>
</template>

<script>
import CartControll from '../../components/Shops/CartControll';
import BScroll from 'better-scroll';
import ShopCart from './ShopCart';
import FoodDetails from './FoodDetails';
// import EventBus from '../../EventBus.js';

export default {
    name: 'Goods',
    data() {
        return {
            shopInfo: null,
            menuScroll: {}, // 左侧滚动对象
            foodScroll: {}, // 右侧滚动对象
            scrollY: 0, // 右侧菜单当前滚动到的y值
            currentIndex: 0,
            listHeight: [], // 12个区列表高度
            selectedFood: null,
            showFood: false,
            foodScrollEnable: false,
            timer: null
        };
    },
    created() {
        this.getData();
    },
    mounted() {
        document
            .getElementsByClassName('shop')[0]
            .addEventListener('scroll', this.switchFoodScrollEnable);
    },
    watch: {
        scrollY(newVal) {
            if (newVal < 0) {
                this.currentIndex = 0;
                return;
            }
            for (let i = 0; i < this.listHeight.length - 1; i++) {
                const height1 = this.listHeight[i];
                const height2 = this.listHeight[i + 1];
                if (newVal >= height1 && newVal < height2) {
                    this.currentIndex = i;
                    return;
                }
            }
            this.currentIndex = this.listHeight.length - 2;
        }
    },
    methods: {
        getData() {
            this.shopInfo = this.$store.getters.CBShop;
            this.$nextTick(() => {
                this.initScroll();
                this.calculateHeight();
            });
        },
        switchFoodScrollEnable() {
            if (!document.getElementById('menuview')) {
                return;
            }
            if (
                document.getElementById('menuview').getBoundingClientRect()
                    .top > document.getElementById('goodsNav').offsetHeight
            ) {
                this.foodScroll.disable();
            } else {
                this.foodScroll.enable();
            }
        },
        initScroll() {
            this.menuScroll = new BScroll(this.$refs.menuScroll, {
                click: true,
                mouseWheel: true
            });

            this.foodScroll = new BScroll(this.$refs.foodScroll, {
                probeType: 3,
                click: true,
                mouseWheel: true,
                bounce: false
            });

            this.foodScroll.on('scroll', pos => {
                // console.log(this.foodScroll.movingDirectionY);
                // console.log(this.scrollY);
                this.scrollY = Math.abs(Math.round(pos.y));
                if (
                    this.foodScroll.movingDirectionY == -1 &&
                    this.scrollY == 0
                ) {
                    // document.getElementsByClassName('shop')[0].scrollTop = 0;
                    clearInterval(this.timer);
                    this.scrollTop();
                    // clearInterval(this.timer)
                }
            });
            this.foodScroll.disable();
        },
        scrollTop() {
            this.timer = setInterval(() => {
                document.getElementsByClassName('shop')[0].scrollTop -= 10;
                if (document.getElementsByClassName('shop')[0].scrollTop <= 0) {
                    document.getElementsByClassName('shop')[0].scrollTop = 0;
                    clearInterval(this.timer);
                }
            }, 1);
        },
        selectMenu(index) {
            // console.log(index);
            this.currentIndex = index;
            const foodlist = this.$refs.foodScroll.getElementsByClassName(
                'food-list-hook'
            );

            const el = foodlist[index];
            // console.log(el);
            this.foodScroll.scrollToElement(el, 250);
        },
        calculateHeight() {
            const lists = this.$refs.foodScroll.getElementsByClassName(
                'food-list-hook'
            );
            let height = 0;
            this.listHeight.push(height);
            for (let i = 0; i < lists.length; i++) {
                const item = lists[i];
                height += item.clientHeight;
                this.listHeight.push(height);
            }
            // console.log(this.listHeight);
        },
        handleFood(food) {
            // console.log(food);
            this.selectedFood = food;
            this.showFood = true;
        }
    },
    components: {
        CartControll,
        ShopCart,
        FoodDetails
    }
};
</script>

<style scoped>
.goods {
    position: relative;
    height: calc(100% - 10.667vw);
}

.recommend {
    padding-top: 4.267vw;
    background-color: #fff;
}
.recommend-name {
    padding-left: 4.267vw;
    color: #333;
    font-size: 0.16rem;
    font-weight: 700;
    margin-bottom: 2.667vw;
}
.recommend-wrap {
    overflow-x: scroll;
    display: flex;
    width: 100%;
}
.recommend-wrap ul {
    display: flex;
    padding-left: 4.267vw;
}
.recommend-wrap ul li {
    flex: none;
    width: 32vw;
    margin-right: 2.667vw;
}
.recommend-wrap li img {
    display: block;
    width: 32vw;
    height: 32vw;
    border-top-left-radius: 0.8vw;
    border-top-right-radius: 0.8vw;
    max-width: 100%;
}
.recommend-food .recommend-food-name {
    color: #333;
    font-size: 0.13rem;
    line-height: 0.15rem;
    margin: 1.867vw 0 1.23vw;
    white-space: nowrap;
    text-overflow: ellipsis;
    overflow: hidden;
}
.recommend-food .recommend-food-zm {
    color: #999;
    font-size: 0.096rem;
    margin-bottom: 1.867vw;
    min-height: 1em;
}
.recommend-food-price {
    /* height: ; */
    display: flex;
    align-items: center;
    justify-content: space-between;
    padding-right: 0.267vw;
}
.recommend-food-price p {
    font-size: 0.16rem;
    color: #ff5339;
}

::-webkit-scrollbar {
    width: 0 !important;
}

.menuview {
    box-sizing: border-box;
    height: 100%;
    padding-bottom: 10.8vw;
    background-color: #fff;
    display: flex;
}
.menu-wrapper {
    overflow-y: hidden;
    /* height: 100%; */
    height: calc(100% - 12.8vw);
    background-color: #f8f8f8;
    padding-bottom: 10.667vw;
    width: 20.53vw;
}
.menu-wrapper li {
    padding: 4.667vw 2vw;
    font-size: 0.096rem;
    color: #666;
    line-height: 1.2;
}
.menu-wrapper li img {
    max-width: 100%;
    width: 3.466667vw;
    height: 3.466667vw;
    vertical-align: top;
    margin-right: 0.096rem;
}

.foods-wrapper {
    overflow-y: hidden;
    /* height: 100%; */
    height: calc(100% - 12.8vw);
    width: 79.467vw;
    padding-bottom: 10.667vw;
}
.category-title {
    margin-left: 2.67vw;
    padding: 2vw 8vw 2vw 0;
    display: flex;
    align-items: center;
    overflow: hidden;
}
.category-title strong {
    margin-right: 1.33vw;
    font-weight: 700;
    font-size: 0.128rem;
    color: #666;
    flex: none;
}
.category-title span {
    flex: 1;
    color: #999;
    font-size: 0.096rem;
    white-space: nowrap;
    overflow: hidden;
    text-overflow: ellipsis;
}
.fooddetails {
    min-height: 30.667vw;
    padding: 2.667vw 0 2.6667vw 2.667vw;
    margin-bottom: 0.133vw;
    display: flex;
}
.fooddetails img {
    width: 25.33vw;
    height: 25.33vw;
    flex: none;
    margin-right: 2.667vw;
    border-radius: 0.53vw;
}
.fooddetails-info {
    flex: 1;
    padding-bottom: 6.667vw;
    padding-right: 4vw;
}
.fooddetails-info h4 {
    padding-right: 4vw;
    font-weight: 700;
    overflow: hidden;
    font-size: 0.16rem;
    line-height: 0.18rem;
    white-space: nowrap;
    width: 40vw;
    text-overflow: ellipsis;
    color: #333;
}
.fooddetails-des {
    margin: 1.33vw 0;
    font-size: 0.096rem;
    color: #999;
    overflow: hidden;
    text-overflow: ellipsis;
    white-space: nowrap;
    width: 42.667vw;
}
.fooddetails-sales {
    margin: 1.73vw 0 !important;
    color: #999;
    font-size: 0.096rem;
    line-height: 1;
    min-height: 1em;
}
.fooddetails-price {
    display: flex;
    align-items: center;
    justify-content: space-between;
    margin-top: 3.73vw;
}
.fooddetails-price .price {
    font-size: 0.16rem;
    line-height: 4.267vw;
    color: #ff5339;
    padding-bottom: 0.93vw;
    display: flex;
    align-items: baseline;
}

.menu-wrapper .current {
    background-color: #fff !important;
    color: #333 !important;
}
</style>
